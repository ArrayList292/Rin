...
exports.run = async (client, msg, args, options) => {
    ...
    if (!msg.member.voice.channel && !msg.guild.me.voice.channel) return msg.channel.send(error1);

    if (!msg.member.voice.channel.joinable) return msg.channel.send(error4);

    if (!args[0]) return msg.channel.send(error2);

    let voiceConnection = await msg.member.voice.channel.join();

    if (!msg.guild.me.deaf) msg.guild.me.voice.setSelfDeaf(true);
    ...
    if (ytdl.validateURL(args[0]) || ytdl.validateID(args[0])) {
        if (ytpl.validateURL(args[0])) {
            let playlist = ytpl(args[0]);

            let waiting1 = new Discord.MessageEmbed()
                .setColor(9472474)
                .setTimestamp(Date.now())
                .setAuthor(`Adding playlist to the queue! Please wait!`, msg.author.avatarURL());

            let waitMsg = msg.channel.send(waiting1);
            for (const i of playlist.items) {
                try {
                    ...
                    newData.message = msg;
                    options.queue.get(msg.guild.id).musics.push(newData);
                } catch (e) {
                    break;
                }
            }
            ...
            let success1 = new Discord.MessageEmbed()
                ...
                .addField("Duration",
                    videoData.duration.timestamp === "0"
                        ? "LIVE"
                        : videoData.duration.timestamp, true);
